...
- `markpad`, `markpad_0.12.0_amd64.deb`, and `Markpad.AppImage` for Linux.
...
snapcraft
snap install --dangerous ./markpad_0.12.0_amd64.snap
```
...
```sh
snapcraft login
snapcraft upload markpad_0.12.0_amd64.snap --release=stable
```
...
The Flatpak manifest needs the immutable commit for the published `v0.12.0` tag and vendored Go sources. Generate those after the tag exists, validate with `flatpak-builder`, and submit the resulting manifest to Flathub. Do not guess the commit ID in advance.
...
2. Create a new `0.12.0` manifest directory from the previous version.
...
After the Windows installer exists, update `packaging/scoop/markpad.json` with version `0.12.0`, the exact release URL, and the real SHA-256. Validate installation from a test bucket before publishing it. The repository copy is only a template while its hash is a placeholder; do not advertise it as installable.

## Final checklist

- `make setup`, `make check`, and `git diff --check` passed before tagging.
- Version and 2026-09-02 release date are synchronized in source and non-hash packaging metadata.
