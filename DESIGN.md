...
- Settings is an overlay inspector with Appearance, Writing, Keyboard, Files, and Help & updates categories. Changes save automatically, render immediately, and never mutate document content. It is fully unmounted when closed. Opening it closes History, Search, palette, and modal surfaces; Escape closes it. Help is also a visible title-bar button and an F1/More action; About and version history are reachable from Help.
