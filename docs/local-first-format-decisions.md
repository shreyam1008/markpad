...
- All scope should de-duplicate equivalent loaded/local hits by normalized path, line, and snippet while preferring loaded buffers.
- Phase 2 sync scope: index remains local per device; sync transfers files and metadata, not search cache.
- Main view and result exports: summarize result count, loaded/local distribution, All-scope de-duplication, and parsed operator counts from the current result page only.
- Search Profile: report current result-page bytes, snippet bytes, and All-scope de-duplication counts from already-rendered results only.
...
- Canvas Guide should explain viewport culling and path bounds so low-memory behavior is discoverable without requiring a profiler.
...
- Main view: show whether expired cleanup candidates exist so disabled cleanup controls are understandable before any destructive action.
