...
- Sticky notes: store as native JSON elements with text, fill, stroke, position, and size; derive readable fill from the existing color control; expose via toolbar and command palette; open text editing immediately after creation; include sticky text in selected-element reports; and export as visible text/card content where adapters support it.
