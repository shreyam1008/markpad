...
- Sticky notes: store as native JSON elements with text, fill, stroke, position, and size; expose via toolbar and command palette; open text editing immediately after creation; include sticky text in selected-element reports; and export as visible text/card content where adapters support it.
