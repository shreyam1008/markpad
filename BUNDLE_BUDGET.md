...
| Stripped Windows binary | 16,143,872 bytes (15.40 MiB) | hard ceiling 16 MiB |
| Frontend JavaScript | 3,957,871 bytes (3.77 MiB) raw, code-split | avoid unbounded growth |
| Frontend CSS | 104,041 bytes (101.6 KiB) raw | generated by Tailwind |

Sizes were measured from the v0.11.0 release candidate on 2026-09-01. The redesigned workbench, modern Markdown/Mermaid support, unified assets, and strengthened interaction contracts remain inside the 16 MiB binary ceiling.

The Windows figures were re-measured on 2026-09-01 after the workbench redesign and modern Markdown pass. Full Mermaid 11 support adds about 3.29 MiB to the stripped Windows binary, so the hard ceiling moved from 15 MiB to 16 MiB. Mermaid is split into lazy chunks and is not fetched or evaluated until a document contains a Mermaid fence; ordinary notes retain the smaller startup path. The runtime remains offline and does not ship a browser engine.
