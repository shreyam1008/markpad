...
- Current backend slice: choose a default local folder, create Markdown notes and native `.markcanvas.json` files there, list bounded files, and perform capped streaming search without watchers or a persistent index.
...
- Current frontend/backend slice: search palette supports Loaded and Local Folder scopes, using bounded backend methods for both.
