...
if grep -Eqi 'File[[:space:]]+Edit[[:space:]]+View[[:space:]]+Settings[[:space:]]+Help' dist/linux-ui-smoke.txt; then
  cat dist/linux-ui-smoke.txt
  echo "Markpad rendered a redundant native Linux application menu above its custom title bar" >&2
  exit 1
fi

echo "Linux UI smoke passed: visible Markpad content rendered without a duplicate native menu."
