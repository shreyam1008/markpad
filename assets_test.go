...
func TestCreateMenuFilesFirstWorkflowIsGuarded(t *testing.T) {
	...
	assertTextIncludesAll(t, "frontend/index.html", string(indexHTML), []string{
		...
		`data-menu-chip="TASKS"`,
		`data-menu-chip="CANVAS"`,
		`data-create-kind="open-folder"`,
		`data-create-kind="search-folder"`,
		`data-create-kind="other"`,
		`data-menu-chip="TXT"`,
		"Local-first",
		`class="create-menu-section-label">Primary files</div>`,
		`class="create-menu-section-label">Markdown templates</div>`,
		`class="create-menu-section-label">Workspace</div>`,
		"Today in the local folder",
		"This week in the local folder",
		"Open or create local <code>Tasks.md</code>, then show list/calendar/kanban views",
		"Create a local <code>.markcanvas.json</code>; use Canvas workflows for scratch boards",
		"Create .markcanvas.json in local folder",
		"Create <code>.txt</code>, JSON, CSV, code, or config text files in the local folder",
		...
		"Export .markcanvas",
		"Download the native canvas file from scratch or active board",
	...
	assertTextIncludesAll(t, "docs/local-first-feature-decisions.md", string(featureDecisions), []string{
		...
		"Task file is a file workflow: open or create `Tasks.md`, then show list, calendar, and kanban views over Markdown task lines from files.",
...
func TestContextMenuDraftSaveAsIsGuarded(t *testing.T) {
	...
	assertTextIncludesAll(t, "frontend/index.html", string(indexHTML), []string{
		`id="ctx-menu"`,
		`role="menu" tabindex="-1" aria-label="File actions"`,
		`type="button" role="menuitem"`,
		`class="ctx-hint"`,
		`class="ctx-separator" role="separator"`,
		`data-ctx="open"`,
		"Open File",
		`data-ctx="search"`,
		"Search Related",
	...
	assertTextIncludesAll(t, "frontend/src/main.js", string(mainJS), []string{
		...
		"onCtx('open', async () => {",
		"await openLocalFolderFile(path);",
		"onCtx('search', async () => {",
		"await showLocalFolder(query);",
		"onCtx('saveas', async () => {",
		"onCtx('canvas', async () => {",
