 ...
 *
 * The desktop bridge exposes a `zen` object on `window` with the
 * same method shape used by the web client. This implementation backs
 * that shape with HTTP calls to the Go server. Swapping this object is the
...
const WEB_APP_INFO: ZenAppInfo = {
  ...
  homepage: 'https://github.com/shreyam1008/shreyamnotes/releases/latest',
  runtime: 'web'
}
...
function getWailsRuntime(): WailsRuntime | null {
  const candidate = (window as Window & { runtime?: WailsRuntime }).runtime
  return candidate && typeof candidate === 'object' ? candidate : null
}

function isWailsDesktop(): boolean {
  return typeof getWailsRuntime()?.WindowMinimise === 'function'
}

function getCapabilities(): ZenCapabilities {
  if (!isWailsDesktop()) return WEB_CAPABILITIES
  return {
    ...WEB_CAPABILITIES,
...
function getAppInfo(): ZenAppInfo {
  if (!isWailsDesktop()) return WEB_APP_INFO
  return {
    ...WEB_APP_INFO,
    name: 'shreyamnotes',
    runtime: 'desktop'
...
function wrapRouteUpgradeError(path: string, err: unknown): never {
  if (
    ...
    throw new Error(
      'Your ShreyamNotes server is running an older build and does not support the new vault picker yet. Restart `bun run dev:server` and reload the page.'
    )
  }
  ...
  if (!res.ok) {
    if (res.status === 401) {
      throw new HttpRequestError(
        res.status,
        path,
        'This ShreyamNotes server requires you to sign in with its auth token.'
      )
    }
...
async function pickVault(): Promise<VaultInfo | null> {
  ...
  const nextPath = window.prompt(
    'Enter the path to the vault directory on the server running ShreyamNotes.',
    suggested
  )
...
function listNotesPage(request: ListNotesPageRequest): Promise<ListNotesPageResponse> {
  return jsonRequest<ListNotesPageResponse>('/notes/page', {
    method: 'POST',
    body: {
      requestId: request.requestId,
      offset: request.offset,
      limit: request.limit ?? request.chunkSize,
...
async function exportNotePdf(_relPath: string): Promise<string | null> {
  ...
  if (!exportWindow) {
    throw new Error(
      'ShreyamNotes could not open the PDF export window. Allow pop-ups for this site and try again.'
    )
  }
  exportWindow.focus()
  return null
...
// @shared/databases) over HTTP so the on-disk format is byte-identical across
// local and server-backed vaults. Reads/writes use the
...
function windowToggleMaximize(): void {
  getWailsRuntime()?.WindowToggleMaximise?.()
}

function windowClose(): void {
  getWailsRuntime()?.Quit?.()
}
...
function clipboardReadText(): string {
  ...
  // synchronous clipboard bridge. Return empty string; callers that
