...
func TestListNotesCapsLargeMarkdownMetadataRead(t *testing.T) {
	root := t.TempDir()
	v, err := New(root, Options{})
	if err != nil {
		t.Fatal(err)
	}
	rel := filepath.ToSlash(filepath.Join(string(FolderInbox), "large.md"))
	abs := filepath.Join(v.Root(), filepath.FromSlash(rel))
	body := strings.Builder{}
	body.WriteString("# Large\n\n#early\n\n")
	body.WriteString(strings.Repeat("x", noteMetadataBodyLimit+1024))
	body.WriteString("\n#late\n")
	...
	if !stringSliceContains(meta.Tags, "early") {
		t.Fatalf("large note metadata lost early tag: %#v", meta.Tags)
	}
	if stringSliceContains(meta.Tags, "late") {
		t.Fatalf("large note metadata parsed beyond startup byte cap: %#v", meta.Tags)
	}
}
...
func TestListNotesPagePaginatesAndFilters(t *testing.T) {
	root := t.TempDir()
	inbox := filepath.Join(root, string(FolderInbox))
	if err := os.MkdirAll(filepath.Join(inbox, "Books.base"), 0o700); err != nil {
		t.Fatal(err)
	}
	for name, body := range map[string]string{
		...
		"Bravo.md":               "# Bravo\n",
		"Charlie.md":             "# Charlie\n",
		"Books.base/Internal.md": "# Hidden\n",
		"Books.base/schema.json": "{}",
		"Books.base/data.csv":    "id,title\n",
		"Ignored.txt":            "not a note",
	} {
		if err := os.WriteFile(filepath.Join(inbox, filepath.FromSlash(name)), []byte(body), 0o600); err != nil {
			t.Fatal(err)
		}
	}
	if err := os.MkdirAll(filepath.Join(root, string(FolderArchive)), 0o700); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(root, string(FolderArchive), "Archived.md"), []byte("# Archived\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	page, err := v.ListNotesPage(ListNotesPageRequest{
		Offset: 1,
		Limit:  1,
		Filter: ListNotesPageFilter{Folder: FolderInbox},
		Sort:   "name-asc",
	})
	if err != nil {
		t.Fatal(err)
	...
	if len(page.Notes) != 1 || page.Notes[0].Title != "Bravo" || page.Notes[0].SiblingOrder != 1 {
		t.Fatalf("unexpected page notes: %#v", page.Notes)
	}

	last, err := v.ListNotesPage(ListNotesPageRequest{
		Offset: 2,
		Limit:  5,
		Filter: ListNotesPageFilter{Folder: FolderInbox},
		Sort:   "name-asc",
	...
	if last.Total != 3 || last.NextOffset != 3 || !last.Done || last.HasMore {
		t.Fatalf("unexpected final page metadata: %#v", last)
	}
	if len(last.Notes) != 1 || last.Notes[0].Title != "Charlie" {
		t.Fatalf("unexpected final page notes: %#v", last.Notes)
	}

	matched, err := v.ListNotesPage(ListNotesPageRequest{
		Limit: 5,
		Query: "char",
	})
	if err != nil {
	...
	if matched.Total != 1 || len(matched.Notes) != 1 || matched.Notes[0].Title != "Charlie" {
		t.Fatalf("unexpected query page: %#v", matched)
	}
}

func TestListNotesPageRequestIDKeepsStableSnapshot(t *testing.T) {
	root := t.TempDir()
	inbox := filepath.Join(root, string(FolderInbox))
	...
	for _, name := range []string{"Alpha.md", "Bravo.md"} {
		if err := os.WriteFile(filepath.Join(inbox, name), []byte("# "+strings.TrimSuffix(name, ".md")+"\n"), 0o600); err != nil {
			t.Fatal(err)
		}
	}
	...
	first, err := v.ListNotesPage(ListNotesPageRequest{
	...
	if first.Total != 2 || len(first.Notes) != 1 || first.Notes[0].Title != "Alpha" || first.Done {
		t.Fatalf("unexpected first page: %#v", first)
	}

	if err := os.WriteFile(filepath.Join(inbox, "Aaron.md"), []byte("# Aaron\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	second, err := v.ListNotesPage(ListNotesPageRequest{
		RequestID: "snapshot-1",
	...
	if second.Total != 2 || len(second.Notes) != 1 || second.Notes[0].Title != "Bravo" || !second.Done {
		t.Fatalf("paginated request did not keep a stable snapshot: %#v", second)
	}

	fresh, err := v.ListNotesPage(ListNotesPageRequest{
	...
	if fresh.Total != 3 || len(fresh.Notes) != 3 || fresh.Notes[0].Title != "Aaron" {
		t.Fatalf("new request did not see fresh files: %#v", fresh)
	}
}
