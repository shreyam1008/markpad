...
const (
	...
	noteMetadataBodyLimit = 256 * 1024
...
type Vault struct {
	...
	metaCacheLoad bool
	metaCacheGen  uint64
	notePageMu    sync.Mutex
	notePageCache map[string][]notePageCandidate
	notePageOrder []string
}
...
func (v *Vault) readMeta(folder NoteFolder, abs string) (NoteMeta, error) {
	...
	body, err := readNoteMetadataBody(abs, info.Size())
	if err != nil {
		return NoteMeta{}, err
	}
	bodyStr := string(body)
...
func readNoteMetadataBody(abs string, size int64) ([]byte, error) {
	if size >= 0 && size <= noteMetadataBodyLimit {
	...
	defer file.Close()
	return io.ReadAll(io.LimitReader(file, noteMetadataBodyLimit))
}
...
const welcomeNote = `# Welcome to ShreyamNotes

ShreyamNotes keeps your notes as plain markdown files in this folder. Use the
sidebar, search, and shortcuts to move quickly, or start typing to begin.

- Notes live in ` + "`inbox/`" + `, ` + "`quick/`" + `, ` + "`archive/`" + `, and ` + "`trash/`" + `.
- Every word you write stays on disk, under your control.
- Vim-style navigation is optional; core shortcuts stay available.
`
