...
const excerptSourceWindowBytes = 256 * 1024
...
func excerptSourceWindow(body string) string {
	if len(body) <= excerptSourceWindowBytes {
	...
	if strings.HasPrefix(source, "---\n") {
		source = stripLeadingFrontmatterForExcerpt(source)
	}
	if len(source) > excerptSourceWindowBytes {
		return source[:excerptSourceWindowBytes]
	}
	return source
}

func stripLeadingFrontmatterForExcerpt(body string) string {
	const opening = "---\n"
...
func BuildExcerpt(body string) string {
	text := stripCodeContent(excerptSourceWindow(body))
