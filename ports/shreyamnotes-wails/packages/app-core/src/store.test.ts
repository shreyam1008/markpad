...
describe('note prefetch memory budget', () => {
  it('skips oversized notes during prefetch', async () => {
    const readNote = vi.fn(async (path: string) => makeNote(`body for ${path}`))
    installZen({ readNote })

    const { useStore } = await loadStore()
    useStore.setState({
      notes: [
        { ...makeNote('small'), path: 'inbox/Small.md', title: 'Small', size: 128 },
        {
          ...makeNote('large'),
          path: 'inbox/Large.md',
          title: 'Large',
    ...
    useStore.getState().prefetchNotes(['inbox/Large.md', 'inbox/Small.md'])
    await flushAsyncWork()

    expect(readNote).toHaveBeenCalledTimes(1)
    expect(readNote).toHaveBeenCalledWith('inbox/Small.md')
    expect(useStore.getState().noteContents['inbox/Large.md']).toBeUndefined()
    expect(useStore.getState().noteContents['inbox/Small.md']).toBeTruthy()
  })
})
...
describe('database deletion', () => {
  it('forgets a deleted database instead of re-reading the gone file', async () => {
    ...
    // logs a native handler error in the terminal).
