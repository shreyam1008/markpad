...
export const HELP_QUICK_START: HelpCard[] = [
  {
    ...
    body:
      'A vault is just a folder of markdown files. ShreyamNotes reads and writes that folder directly, so your notes stay portable, syncable, and readable outside the app too.'
  },
  {
  ...
  {
    title: 'Use files without leaving ShreyamNotes',
    body:
      'Images, SVGs, PDFs, audio, video, and other local files can appear in the vault tree and open in ShreyamNotes tabs or reference panes. The files stay ordinary vault files, but opening them does not have to bounce you out to another app.'
  },
  {
    title: 'Pick up where you left off',
    body:
      'ShreyamNotes restores open tabs, splits, built-in views, and sidebar layout per vault. It also remembers the main window bounds, so reopening the app feels like returning to a workspace rather than starting over.'
  }
]
...
export const HELP_HOW_TO_GUIDES: HelpCard[] = [
  {
    ...
    body:
      'Use the Quick Note shortcut or the command palette entry to create a fast capture note. If date-titled Quick Notes are enabled, ShreyamNotes names it from today’s date automatically; otherwise it creates a normal quick note and focuses the title so you can keep moving.'
  },
  {
  ...
  },
  {
    ...
    body:
      'Open a note in a floating window when you want it in a separate OS window, or pin a note or PDF as a reference when you want it attached to the current writing context inside ShreyamNotes.'
  },
  {
    title: 'Search from Raycast on macOS',
    body:
      'Install the `zen` CLI from Settings → CLI, then use the Raycast Extension section on the same page to install ShreyamNotes for Raycast locally. Raycast can search notes, filter by folder or tag, open a note in the app, open it in a floating window, archive or unarchive, move to Trash, reveal in Finder, copy the note path, and copy a wikilink.'
  },
  {
    title: 'Check for updates and install them',
    body:
      'Use Check for Updates from the app menu, the command palette, or Settings → About. When a release is available, ShreyamNotes can download it in the background and then prompt you to install and relaunch.'
  },
  {
    title: 'Run the self-hosted web version with Docker',
    body:
      'The ShreyamNotes Wails port keeps the server build available for local and hosted testing, but Docker images will be reissued under the ShreyamNotes release pipeline after the port stabilizes. For now, use the desktop build or run the local server from source with a vault folder you control.'
  }
]
...
export const HELP_CORE_CONCEPTS: HelpCard[] = [
  {
    ...
    body:
      'ShreyamNotes edits markdown on disk. Rename, move, archive, restore, and floating-window operations all work on the underlying files, not an internal copy.'
  },
  {
  ...
  },
  {
    ...
    body:
      'When you type `/` at the start of a line or after whitespace, ShreyamNotes opens an inline insert menu for common markdown structures such as headings, bulleted or numbered lists, to-do items, callouts, code blocks, dividers, tables, math blocks, links, images, and even creating a new note page.'
  },
  {
  ...
  },
  {
    ...
    body:
      'Drop files into a note to insert local files. By default, ShreyamNotes keeps them as ordinary files in the vault root, can reveal them from the app, and opens images, SVGs, PDFs, audio, video, and generic files inside ShreyamNotes tabs or reference panes where possible.'
  },
  {
...
export const HELP_SETTINGS: HelpSettingsSection[] = [
  ...
  },
  {
    title: 'Vault',
    items: [
      { label: 'Vault location', detail: 'Reveal or change the root folder ShreyamNotes treats as the active vault.' },
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Raycast on macOS', detail: 'The Raycast extension requires `zen` and can be installed locally from this settings page. ShreyamNotes copies the bundled extension into app data, installs dependencies, builds it, and imports it into Raycast. It searches with `zen list --json`, then opens notes in ShreyamNotes through `zennotes://open` or `zennotes://open-window` and exposes archive, unarchive, trash, reveal, copy path, and copy wikilink actions from Raycast.' },
      { label: 'Uninstall', detail: 'Removes only the ShreyamNotes-managed symlink — never an arbitrary unmanaged binary named `zen`. The CLI stays inside the app bundle for next time.' }
    ]
  },
  {
    title: 'About',
    items: [
      { label: 'App identity', detail: 'See the ShreyamNotes app icon, current version, and a short description of the app as a keyboard-first markdown workflow with Vim motions and plain local files.' },
      ...
      { label: 'Website, community, and issue links', detail: 'The app exposes direct links to the ShreyamNotes project home, discussions, GitHub repository, releases, and issue tracker so support paths stay discoverable.' },
      ...
      { label: 'Source lineage', detail: 'The About section clearly links to the upstream ZenNotes project and the ShreyamNotes project home so fork attribution stays explicit while the app branding remains its own.' }
    ]
  }
]
...
export const HELP_CLI: HelpCard[] = [
  ...
  },
  {
    ...
    body:
      'Open Settings → CLI and click Install. ShreyamNotes symlinks the bundled wrapper into a usable PATH location, preferring user-writable directories and only asking for admin access when no writable PATH target is available. After that, `zen --help` works in any new terminal. You can also run the install from the command palette via "Install Command-Line Tool (zen)".'
  },
  {
    title: 'No app required',
    body:
      'The CLI reads from the same vault folder the desktop app uses, so it works whether or not ShreyamNotes is open. When the app is open, file watchers pick up CLI changes automatically — captures and edits show up live in the sidebar.'
  },
  {
  ...
  },
  {
    ...
    body:
      'On macOS, install the Raycast extension locally from Settings → CLI after `zen` is installed. ShreyamNotes copies the bundled extension into app data, runs the local build, and imports it into Raycast, so you do not need the Raycast Store version. The Search Notes command reads from `zen list --json`, then uses `zennotes://open` to open notes in the main app or `zennotes://open-window` to open a floating window. Cmd+K actions also archive, unarchive, move to Trash, reveal in Finder, copy the path, and copy a wikilink.'
  },
  {
    title: 'Raycast local install requirements',
    body:
      'The local Raycast installer is macOS-only. It needs Raycast, the `zen` CLI, Node.js 22.14 or newer, and Bun 1.3.14 or newer available from your login shell. Settings shows each requirement, the local extension path, and whether the installed copy is current with the bundled ShreyamNotes version.'
  },
  {
  ...
  },
  {
    ...
    body:
      '`zen mcp` starts the ShreyamNotes MCP server in stdio mode — the same one Claude Code, Claude Desktop, and Codex use under the hood. Once `zen` is installed, Settings → MCP installs configure the clients to launch `zen mcp` directly, so the install path is one stable absolute path that survives app moves.'
  }
]
