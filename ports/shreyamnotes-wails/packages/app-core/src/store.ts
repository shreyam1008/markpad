...
import { isExcalidrawPath } from '@shared/excalidraw-path'
...
export type SettingsCategoryTarget =
  | 'appearance'
  | 'editor'
  | 'keymaps'
  | 'typography'
...
// (capped at 1000 there). 500 keeps page JSON transients small enough for
// large-vault startup memory while still avoiding excessive IPC round-trips.
const LIST_NOTES_BRIDGE_PAGE_SIZE = 500
const SIDEBAR_STARTUP_AUTOFOCUS_NOTE_LIMIT = 1000
...
interface Store {
  ...
  settingsTarget: SettingsCategoryTarget | null
  themeId: string
  themeFamily: ThemeFamily
  themeMode: ThemeMode
  editorFontSize: number
  ...
  setSettingsOpen: (open: boolean, target?: SettingsCategoryTarget | null) => void
...
async function ensureWebServerSession(
  ...
  const token = await promptApp({
    ...
    description:
      'This ShreyamNotes server requires its auth token before notes can be accessed in the browser.',
...
function describeWebServerSetupError(err: unknown): string {
  ...
  if (
    ...
    return [
      'ShreyamNotes could not reach its server API.',
      'For normal self-hosted use, run `make up` and open http://localhost:7878.',
      'If you are using the web dev server, make sure `bun run dev:server` is running too.'
    ].join(' ')
  }
  return message
}
...
const MAX_PREFETCHED_NOTE_CONTENTS = 24
const MAX_PREFETCHED_NOTE_BYTES = 2 * 1024 * 1024
const NOTE_PREFETCH_BATCH_SIZE = 12
const MAX_PREFETCH_NOTE_BYTES = 256 * 1024
const MAX_PREFETCH_BATCH_BYTES = 768 * 1024
const INITIAL_VISIBLE_NOTE_PREFETCH_CRITICAL_BATCH_SIZE = 8
const INITIAL_VISIBLE_NOTE_PREFETCH_BACKGROUND_DELAY_MS = 1_600
const INITIAL_VISIBLE_NOTE_PREFETCH_NOTE_LIMIT = 1000
const noteReadPromises = new Map<string, Promise<NoteContent>>()
const prefetchedNotePaths: string[] = []
const prefetchedNoteBytes = new Map<string, number>()
const ASSET_REFRESH_DEDUPE_WINDOW_MS = 10_000
let lastAssetRefreshRoot: string | null = null
let lastAssetRefreshCompletedAt = 0

function didRecentlyRefreshAssetsForVault(root: string): boolean {
  return (
    lastAssetRefreshRoot === root &&
    lastAssetRefreshCompletedAt > 0 &&
    Date.now() - lastAssetRefreshCompletedAt <= ASSET_REFRESH_DEDUPE_WINDOW_MS
  )
}
...
function notePrefetchByteSize(note: Pick<NoteMeta, 'size'>): number | null {
  const size = note.size
  if (!Number.isFinite(size) || size < 0) return null
  return Math.floor(size)
}

function prefetchedContentBytes(): number {
  let total = 0
  for (const bytes of prefetchedNoteBytes.values()) total += bytes
  return total
}

function rememberPrefetchedPath(path: string, byteSize: number): void {
  ...
  prefetchedNoteBytes.set(path, byteSize)
}

function prunePrefetchedContents(s: Store): Partial<Store> {
  if (
    prefetchedNotePaths.length <= MAX_PREFETCHED_NOTE_CONTENTS &&
    prefetchedContentBytes() <= MAX_PREFETCHED_NOTE_BYTES
  ) {
    return {}
  }
  ...
  const contents = { ...s.noteContents }
  const dirty = { ...s.noteDirty }
  let bytes = prefetchedContentBytes()
  while (
    prefetchedNotePaths.length > MAX_PREFETCHED_NOTE_CONTENTS ||
    bytes > MAX_PREFETCHED_NOTE_BYTES
  ) {
    const path = prefetchedNotePaths.shift()
    if (!path) continue
    const pathBytes = prefetchedNoteBytes.get(path) ?? 0
    prefetchedNoteBytes.delete(path)
    const referenced =
      s.selectedPath === path ||
...
function prefetchCandidatePaths(paths: string[], state: Store, batchSize: number): string[] {
  const existing = new Set(Object.keys(state.noteContents))
  const noteByPath = new Map(state.notes.map((note) => [note.path, note]))
  const candidates: string[] = []
  let budgetBytes = 0

  for (const path of paths) {
    if (candidates.length >= batchSize) break
    if (isWorkspaceVirtualTabPath(path) || existing.has(path)) continue
    const note = noteByPath.get(path)
    if (!note) continue

    const size = notePrefetchByteSize(note)
    if (size === null) continue
    if (size > MAX_PREFETCH_NOTE_BYTES) continue
    if (budgetBytes + size > MAX_PREFETCH_BATCH_BYTES) continue
...
async function prefetchInitialVisibleNotes(state: Store): Promise<void> {
  if (state.notes.length > INITIAL_VISIBLE_NOTE_PREFETCH_NOTE_LIMIT) return

  const paths = initialVisibleNotePrefetchPaths(state)
  if (paths.length === 0) return

  const candidates = prefetchCandidatePaths(
    paths,
    state,
    INITIAL_VISIBLE_NOTE_PREFETCH_BATCH_SIZE
  )
...
export const useStore = create<Store>((set, get) => {
  ...
  const scheduleAssetsRefreshForVault = (vault: VaultInfo): void => {
    if (didRecentlyRefreshAssetsForVault(vault.root)) return
    window.setTimeout(() => {
  ...
  prefetchNotes: (paths) => {
    const state = get()
    const candidates = prefetchCandidatePaths(paths, state, NOTE_PREFETCH_BATCH_SIZE)
    ...
    void Promise.all(reads).then((results) => {
      ...
      set((s) => {
        ...
        for (const { path, readScopeKey, content } of loaded) {
          ...
          dirty[path] = false
          rememberPrefetchedPath(path, notePrefetchByteSize(content) ?? content.body.length)
        }
  ...
  refreshAssets: async () => {
    try {
      ...
      lastAssetRefreshRoot = get().vault?.root ?? null
      lastAssetRefreshCompletedAt = Date.now()
  ...
  setSettingsOpen: (open, target) =>
    set((s) => ({
      settingsOpen: open,
      settingsTarget: open ? target ?? (s.settingsOpen ? s.settingsTarget : null) : null
    })),
  ...
  init: async () => {
    ...
    // Default focus to the sidebar so j/k navigation works immediately on
    // normal vaults. Large vaults keep virtualization active at startup; the
    // sidebar still focuses on click/keyboard panel navigation.
    if (
      get().sidebarOpen &&
      !get().focusedPanel &&
      get().notes.length <= SIDEBAR_STARTUP_AUTOFOCUS_NOTE_LIMIT
    ) {
  ...
  openVaultPicker: async () => {
    ...
    try {
      if (!capabilities.supportsLocalFilesystemPickers) {
        const serverCapabilities = await window.zen.getServerCapabilities()
        if (appInfo.runtime === 'web' && !(await ensureWebServerSession(serverCapabilities))) return
        const current = await window.zen.getCurrentVault()
        const enteredPath = await pickServerDirectoryApp(
          {
            title: 'Choose Vault Folder',
            description:
              'Choose the folder on the server that ShreyamNotes should use as your vault.',
  ...
  connectRemoteWorkspace: async () => {
    try {
      ...
      const baseUrl = await promptApp({
        title: 'Connect to ShreyamNotes Server',
        description:
          'Enter the base URL for the ShreyamNotes server, for example `http://localhost:7878` or `https://notes.example.com`.',
      ...
      const authToken = await promptApp({
        ...
        description:
          matchingBaseProfile?.hasCredential
            ...
            : 'If your ShreyamNotes server requires a bearer token, enter it here. Otherwise leave this blank.',
        ...
        const enteredPath = await pickServerDirectoryApp(
          {
            title: 'Choose Vault Folder',
            description:
              'Choose the folder on the connected ShreyamNotes server that should be used as your vault.',
            confirmLabel: 'Choose Folder'
          },
          async (selectedPath) => {
  ...
  changeRemoteWorkspaceVaultPath: async () => {
    try {
      ...
      if (!remoteInfo?.capabilities?.supportsVaultSelection) {
        throw new Error('This ShreyamNotes server does not allow switching vault folders from the app.')
      }
      ...
      const enteredPath = await pickServerDirectoryApp(
        {
          title: 'Choose Vault Folder',
          description:
            'Choose the folder on the connected ShreyamNotes server that ShreyamNotes should use as your vault.',
