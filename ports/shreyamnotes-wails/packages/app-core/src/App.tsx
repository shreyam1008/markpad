...
let editorFocusModulePromise: Promise<typeof import('./lib/editor-focus')> | null = null
...
function focusEditorNormalMode(): void {
  editorFocusModulePromise ??= import('./lib/editor-focus')
  void editorFocusModulePromise.then((module) => module.focusEditorNormalMode())
}
...
const NoteList = lazy(async () => {
  const module = await import('./components/NoteList')
  return { default: module.NoteList }
})

const HomeView = lazy(async () => {
  const module = await import('./components/HomeView')
...
function App(): JSX.Element {
  ...
  const hasOpenWorkspaceTabs = useMemo(
    () => allLeaves(paneLayout).some((leaf) => leaf.tabs.length > 0),
    [paneLayout]
  )
  ...
  useEffect(() => {
    ...
    if (typeof runtimeEvents?.EventsOn !== 'function') return

    const offHandlers = [
      runtimeEvents.EventsOn('wails:menu:new-note', () => {
        void useStore.getState().createAndOpen('inbox', '')
      }),
      runtimeEvents.EventsOn('wails:menu:search-notes', () => {
        setSearchOpen(true)
      }),
      runtimeEvents.EventsOn('wails:menu:about', () => {
        setSettingsOpen(true, 'about')
      })
    ]
  ...
  return (
    <div className="zn-app-shell flex w-screen flex-col bg-paper-100 text-ink-900">
      ...
      <div className="flex min-h-0 flex-1">
        ...
        {!zenMode && noteListOpen && !unifiedSidebar && (
          <Suspense fallback={null}>
            <NoteList />
          </Suspense>
        )}
