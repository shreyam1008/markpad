...
export function CalendarPanel({ note }: { note: NoteContent }): JSX.Element {
  ...
  const dTimer = useRef<ReturnType<typeof setTimeout> | undefined>(undefined)
  ...
  const renderTaskRow = (
    task: VaultTask,
    dayIso: string,
    opts?: { isActive?: boolean; rowRef?: React.RefObject<HTMLDivElement | null> | null }
  ): JSX.Element =>
    editingTaskId === task.id ? (
      ...
      // Draggable <div> (not <button>) — WebKit/Chromium won't fire dragstart
      // on a native button.
      <div
        key={task.id}
