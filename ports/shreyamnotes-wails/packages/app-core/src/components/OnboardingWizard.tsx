...
function BrandMark(): JSX.Element {
  ...
  return (
    <div className="pointer-events-none fixed left-6 top-16 hidden items-center gap-2 text-xs text-ink-500 md:flex">
      ...
      <span className="font-medium tracking-wide">{appInfo.productName}</span>
    </div>
  )
}
  ...
  return (
    <div>
      <StepHeading
        eyebrow="Step 1"
        title="Vim mode"
        subtitle={`${appInfo.productName} keeps Vim-style navigation optional: leader keys, normal/insert, gg/G, and hjkl across the sidebar when you want them. You can change this any time in Settings.`}
      />
  ...
  return (
    <div>
      <StepHeading
        ...
        subtitle={
          isServerVaultSetup
            ? `Pick the folder on the server that ${appInfo.productName} should treat as your vault.`
            : `Pick a folder on this machine. ${appInfo.productName} stores notes as plain .md files — yours to keep, back up, and sync however you like.`
        }
      />
        ...
        <div>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <ChoiceCard
              ...
              title="Inbox folder"
              description={`${window.zen.getAppInfo().productName}'s lifecycle structure: inbox → archive → trash.`}
            />
            <ChoiceCard
