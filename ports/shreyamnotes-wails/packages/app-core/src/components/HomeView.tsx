...
function recentNotesTopK(notes: NoteMeta[], limit: number): NoteMeta[] {
  if (limit <= 0 || notes.length === 0) return []
  const recent: NoteMeta[] = []

  for (const note of notes) {
    if (note.folder === 'trash' || note.folder === 'archive') continue
    let insertAt = recent.length
    while (insertAt > 0 && recent[insertAt - 1].updatedAt < note.updatedAt) insertAt -= 1
...
export function HomeView({
  ...
  const recent = useMemo<NoteMeta[]>(() => recentNotesTopK(notes, MAX_RECENT), [notes])
        ...
        <section>
          ...
          {visibleTasks.length > 0 ? (
            ...
            <button
              ...
              className="mt-1.5 inline-flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-ink-400 transition-colors hover:bg-paper-200/60 hover:text-ink-700 focus:bg-paper-200/70 focus:text-ink-700 focus:outline-none"
            >
              Scan tasks
            </button>
          ) : (
