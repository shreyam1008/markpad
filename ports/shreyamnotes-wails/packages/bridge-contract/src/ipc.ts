...
import type { ZenPlatform } from './bridge'
...
export interface CliInstallStatus {
   ...
   *  dev runs where the desktop bundle has not shipped the CLI yet. */
  available: boolean
...
export interface RaycastExtensionStatus {
  ...
  /** Whether a user-installed Bun binary is visible to login shells. */
  ...
  /** True when Bun meets Raycast's extension tooling requirements. */
  bunMeetsMinimum: boolean
...
export type ExternalFileKind = 'markdown' | 'code' | 'plain'

/** A text file opened from outside any vault (standalone editor window). */
export interface ExternalFileContent {
  /** Absolute path on disk. */
  path: string
  /** File name including extension. */
  name: string
  /** File kind chosen before the editor stack is loaded. */
